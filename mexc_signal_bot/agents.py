...
Requires ALL 4 agents to agree, min 65% confidence
"""
...
class TechnicalAgent(BaseAgent):
    ...
    def calc_rsi(self, closes: List[float], period: int = 14) -> Tuple[float, str, int]:
        if len(closes) < period + 1: return 50.0, "NEUTRAL", 0
        gains, losses = [], []
    ...
    def calc_ema(self, data: List[float], period: int) -> List[float]:
        ...
        ema = [np.nan] * (period - 1); ema.append(sum(data[:period]) / period)
        mult = 2 / (period + 1)
        for price in data[period:]: ema.append((price - ema[-1]) * mult + ema[-1])
        return ema
    def calc_macd(self, closes: List[float]) -> Tuple[str, int]:
        if len(closes) < 35: return "NEUTRAL", 0
        ema12 = self.calc_ema(closes, 12); ema26 = self.calc_ema(closes, 26)
    ...
    def calc_trend(self, candles: List[Dict]) -> Tuple[str, int]:
        ...
        ups = sum(1 for c in candles[-7:] if c["close"] >= c["open"])
    ...
    def analyze(self, candles: List[Dict], current_price: float) -> Dict:
        ...
        up = [c for d, c in votes if d == "UP"]; down = [c for d, c in votes if d == "DOWN"]
        if len(up) > len(down): return {"agent": self.name, "direction": "UP", "confidence": int(sum(up)/len(up)), "reason": f"RSI={rsi_val}, MACD+Trend UP"}
        if len(down) > len(up): return {"agent": self.name, "direction": "DOWN", "confidence": int(sum(down)/len(down)), "reason": f"RSI={rsi_val}, MACD+Trend DOWN"}
...
class MomentumAgent(BaseAgent):
    ...
    def analyze(self, candles: List[Dict], current_price: float) -> Dict:
        ...
        last = candles[-1]; vol = last["volume"]
        avg_vol = statistics.mean([c["volume"] for c in candles[-8:-1]]) or 1
        body = abs(last["close"] - last["open"]); range_ = last["high"] - last["low"] or 1
        body_ratio = body / range_; direction = "UP" if last["close"] > last["open"] else "DOWN"
        vol_ratio = vol / avg_vol; conf, reason = 0, ""
        if vol_ratio > 1.8 and body_ratio > 0.6: conf, reason = 75, f"Strong + volume ({vol_ratio:.1f}x)"
        elif vol_ratio > 1.3 and body_ratio > 0.45: conf, reason = 65, f"Good momentum ({vol_ratio:.1f}x)"
        elif vol_ratio < 0.7: conf, reason, direction = 40, "Weak volume", "NEUTRAL"
        else: conf, reason = 55, "Average momentum"
        closes = [c["close"] for c in candles[-5:]]
        if len(closes) >= 3:
            if closes[-1] > closes[-2] > closes[-3] and direction == "UP": conf = min(85, conf+8); reason += " + accel up"
            elif closes[-1] < closes[-2] < closes[-3] and direction == "DOWN": conf = min(85, conf+8); reason += " + accel down"
...
class StructureAgent(BaseAgent):
    name = "Structure"
    def analyze(self, candles: List[Dict], current_price: float) -> Dict:
        if len(candles) < 20: return {"agent": self.name, "direction": "NEUTRAL", "confidence": 0, "reason": "Not enough data"}
        highs = [c["high"] for c in candles[-12:]]; lows = [c["low"] for c in candles[-12:]]
        higher_highs = highs[-1] > max(highs[:-1]) * 0.998; higher_lows = lows[-1] > min(lows[:-3])
        lower_highs = highs[-1] < max(highs[:-1]) * 1.002; lower_lows = lows[-1] < min(lows[:-3])
        ...
        elif current_price > range_mid * 1.005: direction, conf, reason = "UP", 58, "Upper half of range"
        elif current_price < range_mid * 0.995: direction, conf, reason = "DOWN", 58, "Lower half of range"
...
class PatternAgent(BaseAgent):
    ...
    def analyze(self, candles: List[Dict], current_price: float) -> Dict:
        ...
        if lower_wick > body * 2.1 and lower_wick > upper_wick * 1.7 and body / full_range < 0.35:
            direction, conf = "UP", max(conf, 75); reasons.append("Bullish Pinbar")
        ...
        lows = [x["low"] for x in c[-8:]]; highs = [x["high"] for x in c[-8:]]
        if lows[-1] > lows[-3] and lows[-3] < lows[-5] and last["close"] > last["open"]:
            if direction != "DOWN": direction, conf = "UP", max(conf, 69); reasons.append("Higher Low")
        ...
        if last["low"] <= recent_low * 1.003 and last["close"] > last["open"] and last["close"] > recent_low * 1.003:
            direction, conf = "UP", max(conf, 71); reasons.append("Support bounce")
        ...
        if last["high"] >= recent_high * 0.997 and last["close"] < last["open"] and last["close"] < recent_high * 0.997:
            direction, conf = "DOWN", max(conf, 71); reasons.append("Resistance rejection")
        if all(x["close"] > x["open"] for x in c[-3:]) and direction == "UP": conf = min(83, conf+6); reasons.append("3 green")
        if all(x["close"] < x["open"] for x in c[-3:]) and direction == "DOWN": conf = min(83, conf+6); reasons.append("3 red")
        if conf < 60: direction, conf, reason = "NEUTRAL", 0, "No clear pattern"
...
class MultiAgentSystem:
    ...
    def analyze_pair(self, symbol: str) -> Dict:
        ...
        if len(up_votes) == 3:
            for r in results:
                if r["direction"] == "NEUTRAL" and r.get("confidence", 0) >= 48:
                    r["direction"] = "UP"; r["confidence"] = min(76, max(60, r.get("confidence", 50)+10))
                    r["reason"] = (r.get("reason") or "") + " | team support"; up_votes.append(r)
        if len(down_votes) == 3:
            for r in results:
                if r["direction"] == "NEUTRAL" and r.get("confidence", 0) >= 48:
                    r["direction"] = "DOWN"; r["confidence"] = min(76, max(60, r.get("confidence", 50)+10))
                    r["reason"] = (r.get("reason") or "") + " | team support"; down_votes.append(r)
